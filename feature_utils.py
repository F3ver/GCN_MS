...
def parse_peaks(pk_str):
    """Helper function for converting peak string into vector form.
...
def get_largest_implicit_hydrogen_atoms(mol):
    atoms_implicit_hydrogen_atoms = []
    for atom in mol.GetAtoms():
        atoms_implicit_hydrogen_atoms.append(atom.GetTotalNumHs())
    return max(atoms_implicit_hydrogen_atoms)


def get_largest_implicit_valence(mol):
    atoms_implicit_valence = []
    for atom in mol.GetAtoms():
        atoms_implicit_valence.append(atom.GetImplicitValence())
    return max(atoms_implicit_valence)
...
def make_graph_features(smiles_numpy_arr):
    smiles_array = []
    for smiles in smiles_numpy_arr:
        smiles_array.append(*smiles)
    Parser = GraphParser('./graph_parser_config/config_0.json')
    graphs = Parser.parse_smiles(smiles_array)

    batch = {}
    n_graphs = len(graphs)

    n_nodes = np.array(list(map(lambda g: g['node_features'].shape[0], graphs)))
    n_edges = np.array(list(map(lambda g: g['edge_features'].shape[0], graphs)))
    cum_n_nodes = np.concatenate(([0], np.cumsum(n_nodes)[:-1]))
    cum_n_edges = np.concatenate(([0], np.cumsum(n_edges)[:-1]))

    batch['node_features'] = np.concatenate(list(map(lambda g: g['node_features'], graphs)), axis=0)
    batch['edge_features'] = np.concatenate(list(map(lambda g: g['edge_features'], graphs)), axis=0)
    batch['adj_mat'] = np.concatenate(list(map(lambda g_offset: g_offset[0]['adj_mat'] + g_offset[1],
                                               zip(graphs, cum_n_nodes))), axis=0)
    batch['inc_mat'] = np.concatenate(list(map(
        lambda g_nodeoffset_edgeoffset: g_nodeoffset_edgeoffset[0]['inc_mat'] + [g_nodeoffset_edgeoffset[1],
                                                                                 g_nodeoffset_edgeoffset[2]],
        zip(graphs, cum_n_nodes, cum_n_edges))), axis=0)
    batch['node_graph_map'] = np.repeat(np.arange(n_graphs, dtype=np.int64), n_nodes)
    batch['edge_graph_map'] = np.repeat(np.arange(n_graphs, dtype=np.int64), n_edges)
    degree_list = np.concatenate(list(map(lambda g: g['node_degree'], graphs)))
    degree_mat = np.diag(np.power(degree_list, -0.5)).astype('float32')

    node_features = batch['node_features']
    edge_features = batch['edge_features']
    adj_mat = batch['adj_mat'].astype('int64')
    inc_mat = batch['inc_mat'].astype('int64')
    node_graph_map = batch['node_graph_map'].astype('int32')
    edge_graph_map = batch['edge_graph_map'].astype('float32')
    n_nodes = np.array(node_features.shape[0]).astype('int32')
    num_ext_features = np.array(node_features.shape[1] + edge_features.shape[1]).astype('int32')


    return node_features, edge_features, adj_mat, inc_mat, node_graph_map, edge_graph_map, degree_mat, n_nodes, num_ext_features