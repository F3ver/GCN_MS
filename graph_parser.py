...
class GraphParser(object):
    ...
    def __init__(self, config):
        ...
        #Otherwise, check if config is valid file(assumed to be a JAON file containing the config dictionary)
    ...
    def parse_smiles(self, smiles_array, targets_array=None, ids_array=None, verbose=False):
        ...
        for i in range(n_smiles):
            ...
            g = self.parse_smiles_str(smiles_str=smiles_array[i])
    ...
    def parse_smiles_str(self, smiles_str):
        ...
        adj_mat = np.zeros((2 * n_edges, 2), dtype=np.float32)  # Adjacency matrix (sparse representation)
        inc_mat = np.zeros((2 * n_edges, 2), dtype=np.float32)  # Incidence matrix (sparse representation)
        ...
        g['node_degree'] = np.array([atom.GetDegree() for atom in mol.GetAtoms()])
    ...
    def count_num_features(mode, n_vals, feat_name):
        ...
        if mode == 'category':
            num_features = 1
        # Add as many features as allowable categories if it is to be encoded as one-hot vector
        elif mode == 'one-hot':
            num_features = n_vals
