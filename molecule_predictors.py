...
class MassSpectraPrediction(object):
    ...
    def features_to_load(self, hparams):
        ...
        feature_names = [
            ...
            fmap_constants.INCHIKEY, fmap_constants.SMILES
        ]
    ...
    def make_prediction_ops(self, feature_dict, hparams, mode, reuse=False):
        ...
        with tf.variable_scope('spectrum_predictor', reuse=reuse):
            learned_features = self._make_learned_features(feature_dict, hparams, mode)
...
class MLPSpectraPrediction(MassSpectraPrediction):
    ...
    def _set_model_specific_hparams(self, hparams):
        ...
        hparams.add_hparam('num_hidden_layers', 7)
...
@register_model('gcn')
class GCNSpectraPrediction(MassSpectraPrediction):
    """Prediction with a GraphConvolutional network."""
    ...
    def _make_learned_features(self, feature_dict, hparams, mode):
        ...
        node_features, edge_features, adj_mat, inc_mat, node_graph_map, edge_graph_map, degree_mat, n_nodes, num_ext_features = tf.py_func(
            feature_utils.make_graph_features,
            [feature_dict[fmap_constants.SMILES]],
            [tf.float32, tf.float32, tf.int64, tf.int64, tf.int32, tf.float32, tf.float32, tf.int32, tf.int32])

        n_nodes = tf.cast(tf.shape(node_features)[0], tf.int64)
        n_edges = tf.cast(tf.shape(adj_mat)[0] / 2, tf.int64)
        adj_mat = tf.sparse_to_dense(sparse_indices=adj_mat,
                                     output_shape=tf.stack([n_nodes, n_nodes]),
                                     sparse_values=tf.ones(tf.reshape(tf.cast(2 * n_edges, tf.int32), (1,)), tf.int32))
        adj_mat = adj_mat + tf.diag(tf.ones([tf.shape(adj_mat)[0]], dtype=tf.int32))
        adj_mat = tf.cast(adj_mat, dtype=tf.float32)
        inc_mat = tf.sparse_to_dense(sparse_indices=inc_mat,
                                     output_shape=tf.stack([n_nodes, n_edges]),
                                     sparse_values=tf.ones(tf.reshape(tf.cast(2 * n_edges, tf.int32), (1,)), tf.int32))
        inc_mat = tf.cast(inc_mat, dtype=tf.float32)
        trans_mat = tf.matmul(tf.matmul(degree_mat, adj_mat), degree_mat)

        edge_emb = tf.matmul(inc_mat, edge_features)
        features = tf.concat([node_features, edge_emb], axis=1)
        layer_output = tf.reshape(features, [-1, 69])


        for i in [0, 1]:
            layer_output = tf.matmul(trans_mat, layer_output)
            layer_output = tf.layers.dense(
                inputs=layer_output,
                units=hparams.num_hidden_units)
            layer_output = self._batch_norm(layer_output, is_training=is_training)
            layer_output = activation_fn(layer_output)

        layer_output = tf.segment_sum(data=layer_output, segment_ids=node_graph_map)
